...
def trimmed_videos_annotation(
    ...
    for video_path in video_paths:
        ...
        [trim_start_time, trim_end_time] = [
            float(time) for time in re.findall("\d+", video_path.stem)[-2:]
        ]

        # return start and end of the falling and lying events from original videos annotation file
        fall_start_time = float(
            annotation_file.loc[annotation_file["video_name"] == video_name][
                "fall_start"
            ]
        )
        fall_end_time = float(
            annotation_file.loc[annotation_file["video_name"] == video_name]["fall_end"]
        )
        lying_start_time = float(
            annotation_file.loc[annotation_file["video_name"] == video_name][
                "lying_start"
            ]
        )
        lying_end_time = float(
            annotation_file.loc[annotation_file["video_name"] == video_name][
                "lying_end"
            ]
        )
        ...
        trimmed_videos_annotation_file.append(
            {"video_path": str(video_path), "action_class": action_class}
        )

    # save the annotation dictionary as CSV file in output_folder
    output_file_path = output_folder.joinpath("/trimmed_videos_annotation_file.csv")
